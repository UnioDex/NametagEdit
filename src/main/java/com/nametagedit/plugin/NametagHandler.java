...
public class NametagHandler implements Listener {
	...
	public HashMap<Player, Boolean> isTagVisible = new HashMap<Player, Boolean>();
	...
	public void onQuit(PlayerQuitEvent event) {
		...
		isTagVisible.remove(event.getPlayer());
	}
	...
	public void onPlayerJoin(PlayerJoinEvent event) {
		...
		isTagVisible.put(event.getPlayer(), true);
		new BukkitRunnable() {
			@Override
			public void run() {
	...
	public void onTeleport(final PlayerChangedWorldEvent event) {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				ArrayList<PotionEffectType> pots = new ArrayList<PotionEffectType>();
				for (PotionEffect pot : p.getActivePotionEffects()) {
					pots.add(pot.getType());
				}

				if (pots.contains(PotionEffectType.INVISIBILITY)) {
					if (isTagVisible.get(p)) {
						applyTagToPlayer(event.getPlayer(), false, false);
						isTagVisible.remove(p);
						isTagVisible.put(p, false);
					}
				}else {
					if (!isTagVisible.get(p)) {
						applyTagToPlayer(event.getPlayer(), true, false);
						isTagVisible.remove(p);
						isTagVisible.put(p, true);
					}
				}
			}
	...
	public void onConsume(final PlayerItemConsumeEvent event) {
		...
		if (item.getType().equals(Material.POTION)) {
			for (PotionEffect pot : Potion.fromItemStack(item).getEffects()) {
				pots.add(pot.getType());
			}
		}
	...
	public void onSplash(final PotionSplashEvent event) {
		Collection<LivingEntity> players = event.getAffectedEntities();
		ThrownPotion item = event.getPotion();
		ArrayList<PotionEffectType> pots = new ArrayList<PotionEffectType>();
		for (PotionEffect pot : item.getEffects()) {
			pots.add(pot.getType());
		}
		...
		for (LivingEntity p : players) {
			if (p.getType().equals(EntityType.PLAYER)) {
				if (pots.contains(PotionEffectType.INVISIBILITY)) {
					if (isTagVisible.get((Player)p)) {
						applyTagToPlayer((Player)p, false, false);
						isTagVisible.remove((Player)p);
	...
	public void applyTags() {
		...
		for (Player online : Utils.getOnline()) {
			if (online != null) {
				applyTagToPlayer(online, true, false);
			}
		}
	...
	public void applyTagToPlayer(final Player player, final boolean visible, final boolean loggedIn) {
		// If on the primary thread, run async
		if (Bukkit.isPrimaryThread()) {
			new BukkitRunnable() {
				@Override
				public void run() {
					applyTagToPlayer(player, visible, loggedIn);
				}
			}.runTaskAsynchronously(plugin);
			return;
		}
		...
		new BukkitRunnable() {
			...
			public void run() {
				nametagManager.setNametag(player.getName(), formatWithPlaceholders(player, nametag.getPrefix(), true),
						formatWithPlaceholders(player, nametag.getSuffix(), true), nametag.getSortPriority(), visible);
